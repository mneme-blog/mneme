...
| Transcription ("Transcribe" in the client) fails with **404** | The whisper server does not have its model. Speaches does **not** fetch models on demand — it answers 404 for a model that is not in its cache — which is why the stack ships a one-shot `whisper-model` service that installs it. Check it ran: `./deploy/prod.sh logs whisper-model` (it should print `whisper model ready: …` and exit 0); re-run with `./deploy/prod.sh up -d whisper-model`. It needs outbound network to huggingface.co and downloads ~1.6 GB the first time (it retries five times before giving up, so a single blip is not fatal). From the client side the same fix is in AI settings → Transcription → **Check server** → **Download model**. |
| Installing the model answers **500** with `AttributeError: type object 'tqdm' has no attribute '_lock'` | A known huggingface_hub × tqdm thread-safety bug ([huggingface_hub#3285](https://github.com/huggingface/huggingface_hub/issues/3285)) that kills the download mid-flight. The stack sets `HF_HUB_DISABLE_PROGRESS_BARS=1` on the `whisper` service to avoid it — if you see this, the container predates that setting: `./deploy/prod.sh up -d whisper` to recreate it, then re-run the install. |
| Transcription fails or hangs otherwise | Check the `whisper` container (`logs -f whisper`). Removing both whisper services is fine: clients just get an error and can clear the URL in AI settings. |
...
- **Volumes** (`pgdata`, `miniodata`, `caddy_data`, `caddy_config`, `whisper_models`) hold everything
  (`whisper_models` is only a re-downloadable model cache — dropping it means the next `up` re-runs
  the `whisper-model` install, and transcription 404s until it finishes). `./deploy/prod.sh
